...
cc.Class({
    ...
    spawnNextMonster(waveData) {
        ...
        this.initMonsterByType(monsterType, waveData.level);
    ...
    initMonsterByType(type, level) {
        const monster = this.createMonster(type);
        const worldPos = this.genInitPosison();
        this.setupMonster(monster, worldPos, level);
    },
    createMonster(type) {
    ...
    setupMonster(monsterData, worldPos, level) {
        const { monster, monsterItem } = monsterData;
        const initData = this.getMonsterInitData(monsterItem, level);
        monsterItem.init(initData);
        this.addMonsterToScene(monster, worldPos);
        this.addMonsterToList(monsterItem);
    },
    getMonsterInitData(monsterItem, level) {
        const baseStats = this.calculateBaseStats(level);
        const type = monsterItem.type;
        return {
    ...
    addMonsterToScene(monster, worldPos) {
        this.node.addChild(monster);
        this.setMonsterPosition(monster, worldPos);
    },
